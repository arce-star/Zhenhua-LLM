...
本项目摒弃了传统的文本均匀切分法，采用了**动态滑动窗口**（Dynamic Sliding Window）与**长文引导**（Length-Bias Training）策略：
*   **Contextual Continuity**: 设置 150-200 tokens 的重叠窗口，确保模型捕获跨段落的逻辑关联与叙事节奏。
*   **Length-Bias Training**: 针对性构造 **Short Input (1 Paragraph) -> Long Output (5-8 Paragraphs)** 的映射对，在损失函数中强化对长文本序列的建模能力，有效解决了模型生成“挤牙膏”的问题。
*   **RP-Anchoring (v3.0 Preview)**: 在 Instruction 层级注入角色人格定义（Persona Prompt），实现特定人物（如洛枳、林杨）的对话逻辑锁定。

### 2. Hyperparameter Paradigm (超参数规范)
...
1.  **Stylistic Fidelity (风格忠实度)**：能够自如运用八月长安标志性的“第二人称心理描写”和“通感比喻”。
2.  **Affective Resonance (情感共鸣)**：显著增强了心理描写占比，能细腻处理“卑微与骄傲并存”的复杂暗恋情感。
3.  **Narrative Cohesion (叙事连贯性)**：在长文生成指令下，模型输出的平均 Token 长度提升了 300%，且逻辑中轴稳定。
